...
/* recovery dark:  #7C4DFF
   recovery dxpe:  #9c091d
   recovery light: #F890FF
   recovery lxpe:  #752229
...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    auto& logo = fastbootd_logo_enabled_ ? fastbootd_logo_ : xperience_logo_;
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
  ...
  if (android::base::GetBoolProperty("ro.boot.dynamic_partitions", false) ||
    ...
    xperience_logo_ = LoadBitmap("logo_image_switch");
    fastbootd_logo_ = LoadBitmap("fastbootd");
  } else {
    xperience_logo_ = LoadBitmap("logo_image");
  }
...
int ScreenRecoveryUI::SelectMenu(const Point& point) {
  ...
  if (menu_) {
    if (!menu_->IsMain()) {
      // Back arrow hitbox
      const static int logo_width = gr_get_width(xperience_logo_.get());
      const static int logo_height = gr_get_height(xperience_logo_.get());
